...
if ( ! $release ) {
	error_log(
		date( '[Y-m-d H:i:s]' ) . ' No release response: ' . $_SERVER['REMOTE_ADDR'] . "\n",
		3,
		__DIR__ . '/error_log'
	);
	exit;
...
if ( ! $remote_zip_url ) {
	error_log(
		date( '[Y-m-d H:i:s]' ) . ' Remote zip file is not exist: ' . $_SERVER['REMOTE_ADDR'] . "\n",
		3,
		__DIR__ . '/error_log'
	);
	exit;
...
if ( ! $is_created_package_dir ) {
	error_log(
		date( '[Y-m-d H:i:s]' ) . ' Package directory can not created: ' . $_SERVER['REMOTE_ADDR'] . "\n",
		3,
		__DIR__ . '/error_log'
	);
	exit;
...
if ( ! $is_saved_zip ) {
	error_log(
		date( '[Y-m-d H:i:s]' ) . ' zip file can not created: ' . $_SERVER['REMOTE_ADDR'] . "\n",
		3,
		__DIR__ . '/error_log'
	);
	exit;
...
if ( ! $is_created_response_json ) {
	error_log(
		date( '[Y-m-d H:i:s]' ) . ' response.json is not writable: ' . $_SERVER['REMOTE_ADDR'] . "\n",
		3,
		__DIR__ . '/error_log'
	);
	exit;
...
if ( ! $is_removed ) {
	error_log(
		date( '[Y-m-d H:i:s]' ) . ' GitHub release package can not removed: ' . $_SERVER['REMOTE_ADDR'] . "\n",
		3,
		__DIR__ . '/error_log'
	);
	exit;
